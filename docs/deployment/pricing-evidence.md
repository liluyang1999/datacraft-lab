...
面向东京与新加坡，比较每月运行 60 小时与 730 小时。结论、完整表格和可调计算器见
[云费用报告](../../CLOUD-DEPLOYMENT-ANALYSIS.html)。以下记录原始价格口径与复核方法；不把缺失价格填成零。
...
页面与测试共享 [cloud-costs.js](cloud-costs.js) 的快照数据与计费函数。
