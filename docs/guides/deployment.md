...
同时覆盖按需与常开。当前的综合评估、云平台选择、费用比较与可调计算器见
[项目评估报告](../reports/evaluation-report.html)，价格来源见[价格依据](../reports/pricing-evidence.md)，
云平台决策记录见 [design/decisions.md](../../design/decisions.md)。数据处理契约见
[design/data-contracts.md](../../design/data-contracts.md)，架构见
[design/architecture.md](../../design/architecture.md)，历次改动见 [CHANGELOG](../CHANGELOG.md)。
...
和库存需单独确认，当前自动化容器运行证据是 Linux amd64。完整价格和适用条件只维护在评估报告与
价格依据，不将旧促销、不同地区、不同内存档混作同一比较。
...
[Airflow 指南](airflow.md)。

## 7. 验证与已知范围

以下命令从仓库根目录运行：部署脚本与模板测试、文档一致性测试、费用计算器测试。

```bash
python3 -B -m unittest discover -s tests/deploy -v
python3 -B -m unittest discover -s tests/docs -v
...
`-Djdk.net.unixdomain.tmpdir=${project.build.directory}`（根 POM 属性 `test.jvm.args`）。仍打不开
Selector 的主机上 Spark 套件取消，而 CI 中 Spark 套件出现取消即失败。
