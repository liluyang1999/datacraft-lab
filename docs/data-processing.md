...
Spark and serve small files. Managed Spark jobs in one JVM serialize session ownership, so one
request cannot stop another request's context. An externally owned active session is rejected and
left running.
...
2. The runtime settings. `spark.shufflePartitions` must be a 32-bit integer >= 1
   ...
   launcher `spark.sql.shuffle.partitions` fails with the same message under that name; and
   `spark.enableHive` must be true or false.
...
over `--config`) > `spark-submit --conf spark.sql.shuffle.partitions` > 8. `spark.master` and
`spark.appName` are trimmed, and a blank `spark.master` parameter falls back to the launcher or
`local[*]`. Without an explicit master a job therefore runs on whatever master `spark-submit`
names. The application name is never taken from `spark-submit --name`: it defaults to
`datacraft-lab-<job name>`; set it with `--param spark.appName`.
...
Query keys and values must be percent-encoded UTF-8; `+` in the query decodes to a space:
...
`127.0.0.1` only (`deployment_env.py check` rejects a `datacraft-api` port on any host address
other than `127.0.0.1` or `::1`, and any non-tmpfs `datacraft-api` volume that is not read-only),
Swarm leaves it on the overlay network, and `serve-api` needs `DATACRAFT_DATA_ROOT` off loopback.
Put an authenticated gateway in front before exposing it externally.
