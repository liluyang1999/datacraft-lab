...
SCRIPT = REPOSITORY / ".github" / "scripts" / "check_test_counts.py"
...
class CheckTestCountsTests(unittest.TestCase):
    ...
    def test_module_that_ran_no_tests_fails_and_is_named(self):
        for report in (self.root / "datacraft-io" / "target" / "surefire-reports").iterdir():
            report.unlink()
    ...
    def test_module_without_reports_directory_fails(self):
        reports = self.root / "datacraft-common" / "target" / "surefire-reports"
    ...
    def test_module_with_empty_reports_directory_fails(self):
        for report in (self.root / "datacraft-api" / "target" / "surefire-reports").iterdir():
            report.unlink()
        status, output = run_check(self.root)
        self.assertEqual(1, status, output)
    ...
    def test_scalatest_canceled_tests_do_not_count(self):
        ...
        for report in (self.root / "datacraft-cli" / "target" / "surefire-reports").iterdir():
            report.unlink()
    ...
    def test_every_scalatest_test_canceled_fails(self):
        ...
        for report in (self.root / "datacraft-spark" / "target" / "surefire-reports").iterdir():
            report.unlink()
    ...
    def test_testsuites_root_element_is_summed(self):
        reports = self.root / "datacraft-config" / "target" / "surefire-reports"
        for report in reports.iterdir():
            report.unlink()
    ...
    def test_unreadable_report_fails_without_a_traceback(self):
        reports = self.root / "datacraft-engine" / "target" / "surefire-reports"
        (reports / "TEST-truncated.xml").write_text('<testsuite tests="1"', encoding="utf-8")
        (self.root / "datacraft-cli" / "target" / "surefire-reports" / "TEST-bad.xml").write_text(
    ...
    def test_every_reactor_module_has_a_positive_floor(self):
        ...
        modules = {module.text.strip() for module in pom.findall("{*}modules/{*}module")}
        self.assertTrue(modules)
        self.assertEqual(modules, set(counts.FLOORS))
        for module, floor in counts.FLOORS.items():
